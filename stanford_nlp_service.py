...
class StanfordNER(TypeChecker, object):
  ...
  def __named_entities(self, sentence, _type, group=False):
    if not self._is_string(_type):
      return []
    tags = self.__tags(sentence)
    if self._is_blank(tags):
      return []
    if group is False:
      try:
  ...
  def __tags(self, sentence):
    if self._is_blank(sentence):
      return []
    tokens = self.__tokenize(sentence)
    if not self._is_list(tokens):
      return []
...
class StanfordParser(TypeChecker, object):
  ...
  def trees(self, sentences):
    if self._is_blank(sentences):
      return None
    trees = self.sp.raw_parse_sents(sentences)
    return trees

  def tokens(self, sentence_s):
    if self._is_blank(sentence_s):
      return None
    if self._is_list(sentence_s):
      trees = self.trees(sentence_s)
      tokens = [x.leaves() for x in trees]
      return tokens
    elif self._is_string(sentence_s):
      tree = self.tree(sentence_s)
      tokens = tree.leaves()
      return tokens

  def untokenize(self, tokens, original_sentence):
    if self._is_blank(tokens):
      return None
    if not self._is_list(tokens):
      return None
    if self._is_blank(original_sentence):
      return None
    if not self._is_string(original_sentence):
      return None
    if len(tokens) == 1:
      return tokens[0]
    left_token_map = {
    ...
    if self._is_list(sentence_s):
      ...
      if flatten is True:
        ...
        for i, x in enumerate(phrases):
          for y in x:
            temp.append(self.untokenize(y, sentence_s[i]))
        phrases = temp
    elif self._is_string(sentence_s):
      tree = self.tree(sentence_s)
      ...
      if flatten is True:
        temp = []
        for x in phrases:
          temp.append(self.untokenize(x, sentence_s))
        phrases = temp
    return phrases
